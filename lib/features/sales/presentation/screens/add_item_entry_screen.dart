...
class AddItemEntryScreen extends StatefulWidget {
  // final int fromTabIndex;
  final PendingModel item;
  final int? index;
  final String? modelName;
  const AddItemEntryScreen({
    super.key,
    required this.item,
    this.index,
    this.modelName,
  ...
  @override
  State<AddItemEntryScreen> createState() => _AddItemEntryScreenState();
}

class _AddItemEntryScreenState extends State<AddItemEntryScreen> {
  // Controllers
  ...
  bool inprogressItemEntry = false;
          ...
          child: Form(
            ...
            child: Column(
              children: [
                ...
                if (selectedItemDetails?.hasAttribute == 1 &&
                    isFieldActive("attribute_select")) ...[
                ...
                Visibility(
                  visible: inprogressItemEntry == false,
  ...
  Future<void> _additemEntry() async {
    if (inprogressItemEntry) return;

    inprogressItemEntry = true;
    setState(() {});
    ...
    ApiResponse response = await NetworkCaller.postRequest(
      url: Urls.additemsalesEntryUrl(widget.item.id.toString()),
      body: requestBody,
      token: AuthController.accessToken,
    );
...
//  97. এমন element খুঁজে বের করো যার:
// left side-এর maximum
//         ↓
//      element
//         ↓
// right side-এর minimum

// এর মধ্যে elementটি থাকবে।
